...
function modelFor(config: ProviderConfig) {
...
async function callGemini(config: ProviderConfig, prompt: string) {
  ...
  const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${process.env[config.keyEnv]}`, {
    method: "POST",
...
export async function generateWithFailover(prompt: string, task = "general", requestId?: string): Promise<AiRouterResult> {
  ...
  for (const config of PROVIDERS) {
    ...
    try {
      ...
      if (supabase) await supabase.from("ai_provider_usage").insert({ provider: config.name, task, status: "success", duration_ms: Date.now() - providerStarted, input_tokens: result.usage?.inputTokens ?? null, output_tokens: result.usage?.outputTokens ?? null, request_id: requestId ?? null });
      ...
      if (supabase) await supabase.from("ai_provider_usage").insert({ provider: config.name, task, status: "failed", duration_ms: Date.now() - providerStarted, error_message: lastError.message.slice(0, 1000), request_id: requestId ?? null });
    }
  }

  throw new Error(`No configured AI provider succeeded after ${Date.now() - started}ms. Attempted: ${attempted.join(", ") || "none"}. Last error: ${lastError?.message ?? "no providers configured"}`);
}
