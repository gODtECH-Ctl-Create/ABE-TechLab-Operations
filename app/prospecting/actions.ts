...
export async function createResearchRequest(formData: FormData) {
  ...
  const auditPayload: AuditEventInsert = { actor_type: "human", actor_id: user.id, action: "research.request_created", entity_type: "research_request", entity_id: requestId, metadata: { query, geography, industries, requested_by_role: role } };
  ...
  const workerResponse = await fetch(`${supabaseUrl}/functions/v1/research-prospects-router`, { method: "POST", headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" }, body: JSON.stringify({ requestId }) });
  if (!workerResponse.ok) {
    const workerBody = await workerResponse.text().catch(() => "");
    await supabase.from("research_requests").update({ status: "failed", error_message: workerBody.slice(0, 1000) || `Research worker returned ${workerResponse.status}` } as never).eq("id", requestId);
