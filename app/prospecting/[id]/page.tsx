...
const evidenceItems = (prospect: Prospect): EvidenceObject[] => !Array.isArray(prospect.evidence) ? [] : prospect.evidence.filter((item) => typeof item === "object" && item !== null && !Array.isArray(item)) as EvidenceObject[];
...
export default async function ProspectDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: organisation }, { data: qualification }, { data: lead }, { data: audit }] = await Promise.all([
    supabase.from("organisations").select("id, name, industry, geography, website_url").eq("id", row.organisation_id).maybeSingle(),
  ...
  return <main className="page-shell">
    <header className="page-header"><div><div className="eyebrow">ARIA research · Prospect</div><h1>{org?.name ?? "Prospect"}</h1><p>{org?.industry ?? "Industry not set"} · {org?.geography ?? "Geography not set"}</p></div><Link className="ghost-button" href="/aria/research">← ARIA Research</Link></header>
    <AriaNavigation active="Research" />
