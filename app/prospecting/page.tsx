...
const evidenceItems = (prospect: Prospect) => Array.isArray(prospect.evidence)
  ? prospect.evidence.filter((item): item is Record<string, unknown> => typeof item === "object" && item !== null)
  : [];
  ...
  return (
    <div className="page-shell">
      <header className="page-header">
        <div><div className="eyebrow">ARIA · Customer intelligence</div><h1>Prospect research</h1><p>Define who we should look for. ARIA queues the request, preserves the evidence trail and prepares the next qualification step.</p></div>
        <a className="ghost-button" href="/">← Dashboard</a>
      </header>

      {created ? <div className="success-banner"><strong>Research request queued.</strong><span>The request and audit event are now in Supabase.</span></div> : null}
      ...
      <section className="research-layout">
        <div className="card research-builder">
          <div className="eyebrow">New research run</div><h2>Find potential customers</h2>
          <p className="section-copy">Start narrow. Define the customer type, market and industries so the eventual research worker has a precise target.</p>
          <form action={createResearchRequest} className="research-form">
            <label>What are we looking for?<textarea name="query" required placeholder="Nigerian organisations that need product strategy, software development or AI automation." /></label>
            <div className="research-form-grid"><label>Geography<input name="geography" defaultValue="Nigeria" /></label><label>Industries<input name="industries" placeholder="Education, FinTech, Healthcare" /></label></div>
            <div className="research-checks"><span>ARIA will preserve:</span><span>✓ request</span><span>✓ qualification context</span><span>✓ audit event</span><span>✓ approval boundary</span></div>
      ...
      <section className="card research-queue-card"><div className="section-heading"><div><div className="eyebrow">Research queue</div><h2>Recent requests</h2><p>Live requests waiting for the research worker or showing their processing state.</p></div><span className="badge">{researchRequests.length} requests</span></div>
        <div className="request-list">{researchRequests.length === 0 ? <div className="empty-stage"><strong>No research runs yet</strong><span>Create the first request above.</span></div> : researchRequests.map((request) => <div className="request-row" key={request.id}><div className="request-main"><strong>{request.query}</strong><span>{request.geography ?? "Any geography"} · {Array.isArray(request.industries) && request.industries.length ? request.industries.join(", ") : "Any industry"}</span></div><span className={`status-chip ${request.status === "queued" ? "queued" : ""}`}>{statusLabel(request.status)}</span><small>{new Date(request.created_at).toLocaleString()}</small></div>)}</div>
      </section>
