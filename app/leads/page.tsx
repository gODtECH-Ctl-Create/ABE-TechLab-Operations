  ...
  return (
    <main className="page-shell">
      ...
      <section className="card pipeline-card">
        ...
        <div className="pipeline">{stages.map((stage) => { const stageLeads = visibleLeads.filter((lead) => lead.status === stage.id); return <div className="pipeline-column" key={stage.id}><div className="pipeline-title"><span>{stage.label}</span><b>{stageLeads.length}</b></div>{stageLeads.map((lead) => { const organisation = organisationById.get(lead.organisation_id); return <Link className="lead-card lead-card-link" href={`/leads/${lead.id}`} key={lead.id}><div className="lead-score">{lead.score ?? "--"}</div><h3>{organisation?.name ?? "Unknown organisation"}</h3><p>{lead.service_interest ?? "Service not yet identified"}</p>{lead.problem_summary && <small>{lead.problem_summary}</small>}<small>{lead.source ?? "manual"} · {new Date(lead.created_at).toLocaleDateString()}</small><span className="lead-card-open">Open lead →</span>{(role === "admin" || role === "operator") && <span className="sr-only">Status can be changed from the lead detail page</span>}</Link>;})}{stageLeads.length === 0 && <div className="empty-stage">No leads</div>}</div>; })}</div>
      </section>

      <section className="lead-details-grid">{highPriority.map((lead) => { const organisation = organisationById.get(lead.organisation_id); return <Link className="card lead-detail lead-detail-link" href={`/leads/${lead.id}`} key={lead.id}><div className="eyebrow">Priority lead</div><div className="detail-top"><h2>{organisation?.name ?? "Unknown organisation"}</h2><span className="score-pill">{lead.score}/100</span></div><p>{lead.problem_summary ?? "No problem summary has been recorded yet."}</p><div className="recommended"><strong>Service interest</strong><span>{lead.service_interest ?? "Not assigned"}</span></div><div className="recommended"><strong>Current stage</strong><span>{statusLabels[lead.status] ?? lead.status}</span></div><span className="text-link">Open lead →</span></Link>; })}</section>
    </main>
  );
}
