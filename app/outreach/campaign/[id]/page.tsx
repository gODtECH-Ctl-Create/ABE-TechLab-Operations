...
export default async function OutreachCampaignDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: strategy }, { data: messages }, { data: audit }] = await Promise.all([
  ...
  return <main className="page-shell">
    ...
    <section className="card"><div className="section-heading"><div><div className="eyebrow">Message sequence</div><h2>Draft messages</h2><p>Review the full sequence before any future provider integration.</p></div></div>{!messages?.length ? <div className="empty-stage">No messages drafted for this campaign.</div> : <div className="compact-list">{(messages as Message[]).map(message => <article className="compact-row" key={message.id}><div><strong>{message.subject || "Untitled message"}</strong><span>{message.stage.replaceAll("_", " ")} · {message.status}</span><p className="row-body">{message.body}</p></div><span className="status-chip">{message.status}</span></article>)}</div>}</section>
    <section className="card"><div className="section-heading"><div><div className="eyebrow">Audit trail</div><h2>Campaign activity</h2><p>Approvals, status changes and other operational events.</p></div></div>{!audit?.length ? <div className="empty-stage">No campaign activity recorded yet.</div> : <div className="activity-list">{(audit as AuditEvent[]).map(event => <div className="activity-item" key={event.id}><strong>{event.action.replaceAll("_", " ")}</strong><span>{event.actor_type} · {new Date(event.created_at).toLocaleString()}</span></div>)}</div>}</section>
  </main>;
}
