...
export default async function OpportunityDetailPage({ params, searchParams }: Props) {
  ...
  const [organisationResult, leadResult, auditResult] = await Promise.all([
  ...
  return <main className="page-shell">
    ...
    {error && <div className="error-banner"><strong>Could not update opportunity.</strong><span>{error}</span></div>}
    ...
    {(role === "admin" || role === "operator") && <section className="card"><div className="section-heading"><div><div className="eyebrow">Pipeline control</div><h2>Opportunity control</h2><p>Keep the commercial stage, owner and next action explicit.</p></div></div><form action={updateOpportunity} className="research-form"><input type="hidden" name="opportunity_id" value={row.id} /><div className="research-form-grid"><label>Stage<select name="stage" defaultValue={row.stage}>{stages.map((stage) => <option key={stage} value={stage}>{labels[stage]}</option>)}</select></label><label>Owner user ID<input name="owner_id" defaultValue={row.owner_id ?? user.id} placeholder="Authenticated user UUID" /></label><label>Probability (%)<input name="probability" type="number" min="0" max="100" defaultValue={row.probability ?? ""} /></label></div><div className="research-form-grid"><label>Next action<input name="next_action" defaultValue={row.next_action ?? ""} placeholder="Send proposal, schedule demo..." required={!row.next_action} /></label><label>Next action due<input name="next_action_due_at" type="datetime-local" defaultValue={toLocalInputValue(row.next_action_due_at)} /></label><label>Value (NGN)<input name="value" type="number" min="0" step="1000" defaultValue={row.value ?? ""} /></label></div><button className="primary-button" type="submit">Save opportunity controls</button></form></section>}
    ...
    <section className="card"><div className="section-heading"><div><div className="eyebrow">Accountability</div><h2>Next action</h2><p>Every active opportunity should have a clear owner and a concrete next step.</p></div></div><div className="lead-details-grid"><div className="recommended"><strong>Owner</strong><span>{row.owner_id ?? "Unassigned"}</span></div><div className="recommended"><strong>Action</strong><span>{row.next_action ?? "No next action recorded"}</span></div><div className="recommended"><strong>Due</strong><span>{row.next_action_due_at ? new Date(row.next_action_due_at).toLocaleString() : "No due date"}</span></div></div></section>
