...
export default async function OpportunityDetailPage({ params }: Props) {
  ...
  const [{ data: organisation }, { data: lead }, { data: audit }] = await Promise.all([
    supabase.from("organisations").select("id, name, website, industry, location").eq("id", row.organisation_id).maybeSingle(),
    row.lead_id ? supabase.from("leads").select("id, contact_name, contact_email, service_interest, status").eq("id", row.lead_id).maybeSingle() : Promise.resolve({ data: null }),
  ...
  const org = organisation as Pick<Organisation, "id" | "name" | "website" | "industry" | "location"> | null;
  const linkedLead = lead as Pick<Lead, "id" | "contact_name" | "contact_email" | "service_interest" | "status"> | null;

  return <main className="page-shell">
    <header className="page-header">
      <div><div className="eyebrow">CRM · Opportunity</div><h1>{row.name}</h1><p>{row.description || "No opportunity description has been added yet."}</p></div>
      <Link className="ghost-button" href="/opportunities">← Opportunities</Link>
    </header>

    <section className="lead-summary">
      <div className="summary-card"><span>Stage</span><strong>{labels[row.stage as (typeof stages)[number]] ?? row.stage}</strong></div>
      <div className="summary-card"><span>Value</span><strong>{row.value === null ? "Not set" : money.format(Number(row.value))}</strong></div>
      <div className="summary-card"><span>Probability</span><strong>{row.probability === null ? "--" : `${row.probability}%`}</strong></div>
      <div className="summary-card"><span>Expected close</span><strong>{row.expected_close_date ? new Date(row.expected_close_date).toLocaleDateString() : "Not set"}</strong></div>
    </section>

    {(role === "admin" || role === "operator") && <section className="card">
      <div className="section-heading"><div><div className="eyebrow">Pipeline control</div><h2>Update stage</h2><p>Keep the commercial pipeline aligned with the latest conversation.</p></div></div>
    ...
    <div className="detail-grid">
      <section className="card"><div className="eyebrow">Account</div><h2>Organisation</h2>{org ? <><p><Link className="text-link" href={`/organisations/${org.id}`}>{org.name}</Link></p><dl className="detail-list"><div><dt>Industry</dt><dd>{org.industry || "Not set"}</dd></div><div><dt>Location</dt><dd>{org.location || "Not set"}</dd></div><div><dt>Website</dt><dd>{org.website || "Not set"}</dd></div></dl></> : <p>Organisation not found.</p>}</section>
      <section className="card"><div className="eyebrow">Origin</div><h2>Related lead</h2>{linkedLead ? <><p><Link className="text-link" href={`/leads/${linkedLead.id}`}>{linkedLead.contact_name || linkedLead.contact_email || "View lead"}</Link></p><dl className="detail-list"><div><dt>Email</dt><dd>{linkedLead.contact_email || "Not set"}</dd></div><div><dt>Service</dt><dd>{linkedLead.service_interest || "Not set"}</dd></div><div><dt>Lead status</dt><dd>{linkedLead.status}</dd></div></dl></> : <p>No lead is linked to this opportunity.</p>}</section>
    </div>
