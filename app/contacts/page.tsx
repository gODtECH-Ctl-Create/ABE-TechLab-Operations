...
import { createContact, trashContact } from "./actions";
...
export default async function ContactsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <main className="page-shell">
    ...
    <section className="card"><div className="section-heading"><div><div className="eyebrow">Live records</div><h2>People directory</h2><p>Clear relationship context for every organisation.</p></div><span className="badge">{contacts.length} records</span></div>
      {contacts.length === 0 ? <div className="empty-stage"><strong>No contacts yet</strong><span>Add the first contact above. Contacts tell the team who actually represents an organisation, what their role is, and who influences a decision.</span></div> : <div className="organisation-list">{contacts.map((contact) => { const org = organisationById.get(contact.organisation_id); const name = [contact.first_name, contact.last_name].filter(Boolean).join(" "); return <article className="organisation-row" key={contact.id}><div className="organisation-avatar">{name.slice(0,1).toUpperCase()}</div><div className="organisation-main"><strong>{name}</strong><span>{contact.job_title ?? "Role not set"} · {org?.name ?? "Organisation not found"}</span><small>{contact.email ?? "No email"}{contact.phone ? ` · ${contact.phone}` : ""}</small></div><div className="organisation-meta"><b>{contact.is_decision_maker ? "Decision-maker" : "Contact"}</b><small>{contact.is_decision_maker ? "Buying influence" : "Relationship"}</small></div>{canCreate && <form action={trashContact}><input type="hidden" name="id" value={contact.id} /><input type="hidden" name="reason" value="Removed from active workspace" /><button className="ghost-button" type="submit">Move to Trash</button></form>}</article>; })}</div>}
    </section>
  </main>;
}
