...
export default async function ContactsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const [{ data: contactRows, error: contactsError }, { data: organisationRows, error: organisationsError }] = await Promise.all([
    (supabase.from("contacts") as any).select("id, organisation_id, first_name, last_name, job_title, email, phone, is_decision_maker, notes, created_at").is("deleted_at", null).order("first_name", { ascending: true }).limit(300),
    supabase.from("organisations").select("id, name, industry, geography").order("name", { ascending: true }).limit(300),
  ]);
  if (contactsError) throw new Error(contactsError.message); if (organisationsError) throw new Error(organisationsError.message);
  const contacts = (contactRows ?? []) as Contact[]; const organisations = (organisationRows ?? []) as Organisation[]; const organisationById = new Map(organisations.map((org) => [org.id, org])); const canCreate = ["admin", "operator"].includes(userRole);
  ...
  return <main className="page-shell">
    ...
    <section className="card"><div className="section-heading"><div><div className="eyebrow">Live records</div><h2>People directory</h2><p>Clear relationship context for every organisation.</p></div><span className="badge">{contacts.length} records</span></div>
      {contacts.length === 0 ? <div className="empty-stage"><strong>No contacts yet</strong><span>Add the first contact above. Contacts tell the team who actually represents an organisation, what their role is, and who influences a decision.</span></div> : <div className="contact-directory">{contacts.map((contact) => { const org = organisationById.get(contact.organisation_id); const name = [contact.first_name, contact.last_name].filter(Boolean).join(" "); return <article className="organisation-row record-item contact-row" key={contact.id}><RecordOverflowMenu entity="contact" id={contact.id} href={`/contacts/${contact.id}`} /><div className="organisation-avatar">{name.slice(0,1).toUpperCase()}</div><div className="contact-main"><strong className="contact-name">{name}</strong><span className="contact-context">{contact.job_title ?? "Role not set"} · {org?.name ?? "Organisation not found"}</span><small className="contact-details">{contact.email ?? "No email"}{contact.phone ? ` · ${contact.phone}` : ""}</small><span className="contact-role-badge">{contact.is_decision_maker ? "Decision-maker" : "Contact"}</span></div></article>; })}</div>}
    </section>
  </main>;
}
